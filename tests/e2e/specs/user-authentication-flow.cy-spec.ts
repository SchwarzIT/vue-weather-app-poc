...
describe("Login and state after that", () => {
  it("should successfully log into our app and then access all restricted resources", () => {
    ...
    cy.get(".mode-nav-bar").should($button => {
    ...
    cy.get(".home-nav-header > .hydrated").should("contain", title);
    ...
    cy.get(".container > div > .mode-sidebar")
      .should("have.attr", "href", "country-list")
    ...
    cy.get("@getAllCountries").then(() => {
      ...
      cy.get(":nth-child(1) > .country-entity").should(
        "have.prop",
        "text",
        firstCountry.name.toUpperCase()
      );

      cy.get(":nth-child(2) > .country-entity").should(
        "have.prop",
        "text",
      ...
      cy.get("@getAllCitiesByCountry").then(() => {
        ...
        cy.url().should(
          "eq",
          `http://localhost:8080/city-list/${firstCountry.iso}`
        );

        cy.get(":nth-child(1) > .city-entity").should(
          "have.prop",
          "text",
          `${firstCity.name.toUpperCase()} ${firstCity.temperature}`
        );

        cy.get(":nth-child(2) > .city-entity").should($h1 => {
          expect($h1).to.have.prop(
            "text",
            `${nextCity.name.toUpperCase()} ${nextCity.temperature}`
          );
        });
      ...
      cy.get("input")
        .type("Dusseldorf 11")
        .should("have.value", "Dusseldorf 11")
        .then(() => {
          cy.get(".add-button")
            .click()
            .then(() => {
    ...
    cy.get(".user-profile").then(() => {
      expect(user).to.equal(testUser);
    });

    cy.get('[element="h2"] > .headline > .headline-content').should(
      "contain",
      "Your User Details:"
    );
    ...
    cy.get("#name").should("contain", `Name: ${testUser.name}`);

    cy.get("#email").should("contain", `Email: ${testUser.email}`);
